...
// ProviderHandler handles CRUD operations for email providers.
...
// List handles GET /api/v1/admin/email/providers
func (h *ProviderHandler) List(w http.ResponseWriter, r *http.Request) {
	...
	if providers == nil {
		providers = []model.EmailProvider{}
	}
	apierror.WriteData(w, http.StatusOK, providers, nil)
}

// Get handles GET /api/v1/admin/email/providers/{id}
...
// Create handles POST /api/v1/admin/email/providers
...
// Update handles PUT /api/v1/admin/email/providers/{id}
...
// Delete handles DELETE /api/v1/admin/email/providers/{id}
...
// Test handles POST /api/v1/admin/email/providers/{id}/test
func (h *ProviderHandler) Test(w http.ResponseWriter, r *http.Request) {
	...
	if req.To == "" {
		apierror.WriteJSON(w, http.StatusBadRequest, apierror.APIResponse{
			Success: false,
			Error:   &apierror.APIError{Code: apierror.ErrCodeValidationError, Message: "to email is required"},
		})
		return
	}

	if err := h.providerSvc.TestProvider(r.Context(), id, req.To); err != nil {
		apierror.WriteJSON(w, http.StatusBadRequest, apierror.APIResponse{
			Success: false,
			Error:   &apierror.APIError{Code: apierror.ErrCodeValidationError, Message: err.Error()},
		})
	...
	apierror.WriteData(w, http.StatusOK, map[string]string{"status": "test email sent"}, nil)
}